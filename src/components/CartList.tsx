...
export const CartList = ({ onEditItem }: CartListProps) => {
  const { items, total, updateQuantity, removeItem, validateOrder, clearSentItems } = useCartStore();
        ...
        {pendingItems.length > 0 && (
          <div>
            ...
            {pendingItems.map((item) => (
              <div key={item.id} className="theme-menu-card p-3 rounded-lg">
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="theme-button-secondary w-6 h-6 rounded text-xs flex items-center justify-center"
                    >
                      -
                    </button>
                    <span className="theme-foreground-text text-sm font-medium w-8 text-center">
                      {item.quantite}
                    </span>
                    <button
                      onClick={() => updateQuantity(item.id, item.quantite + 1)}
            ...
            {sentItems.map((item) => (
              <div key={item.id} className="theme-menu-card p-3 rounded-lg opacity-75">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleCancelSentItem(item)}
                      className="text-red-500 hover:text-red-700"
                    >
